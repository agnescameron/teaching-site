...
1. connect to the local wifi network. Look in the list of WiFi networks, and select one called 'fake tattoo'. This is being hosted by a small router called a [Piratebox](https://piratebox.cc/). It won't connect to the wider internet, but it'll let us talk to each other very easily without worrying about proxies.
...
Then, in the handler where you'd like to display the file, replace `res.send()` with `res.sendfile("index.html")`, where 'index.html' here is the filepath. (if it was in a folder called HTML, you'd use `res.sendfile("html/index.html")`)
